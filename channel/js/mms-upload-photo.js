...
// judge text 输入框判断
function judgeText() {
	var texts = mui(".mui-active input[type=text],.mui-active textarea");
	
	if(!ChmJudgeText().judge){
		return judgeObj = ChmJudgeText();
	}else{
		for(var x = 0; x < texts.length; x++) {
			if(!texts[x].value) {
				judgeObj["tips"] = texts[x].tagName == "INPUT" ? "系统名称" : "描述";
...
// 渠道版 表单验证
function ChmJudgeText(){
	var docs = mui("#serviceaplval,#serviceschoolval");
	if(!docs.length){
		judgeObj.judge = true;
		return judgeObj;
	} 
	for(var i = 0; i < docs.length; i++) {
		if(~docs[i].innerHTML.indexOf("选择")){
			judgeObj["tips"] = docs[i].parentElement.querySelector("label").innerHTML.substring("0,1");
			judgeObj.judge = false;
			break;
		}else{
			judgeObj.judge = true;
