...
//		var objtext = "#item1mobile .mui-scroll";
		//循环初始化所有下拉刷新，上拉加载。
		var pages = 0;
...
function messages(type, pages) {
	...
	getAjax(urls, datas, messages_success);
...
function dom_list(data, type) {
	...
	if(type == "message") {
		...
		createI.innerHTML = get_date(data.reg_time, "ymdhm");
	} else {
...
function msg_list_tap(type) {
	mui(".mui-scroll ul").off("tap").on("tap", "a", function() {
		...
		console.log("getids = "+this.getAttribute("getids"))
